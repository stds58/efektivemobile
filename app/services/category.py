...
from app.schemas.category import SchemaCategoryCreate, SchemaCategoryFilter, SchemaCategoryPatch
from app.schemas.permission import AccessContext
from app.exceptions.base import PermissionDenied


async def find_many_category(access: AccessContext, filters: SchemaCategoryFilter, session: AsyncSession):
    if "read_all_permission" in access.permissions:
        return await CategoryDAO.find_many(filters=filters, session=session)
...
async def update_one_category(
    ...
    if "update_all_permission" in access.permissions:
        return await CategoryDAO.update_one(model_id=category_id, session=session, values=filters_dict)
    if "update_permission" in access.permissions:
        return await CategoryDAO.update_one(model_id=category_id, session=session, values=filters_dict)
    raise PermissionDenied(custom_detail="Missing update or update_all permission on product")
...
async def delete_one_category(
    ...
    if "delete_permission" in access.permissions:
        return await CategoryDAO.delete_one_by_id(model_id=category_id, session=session)
    raise PermissionDenied(custom_detail="Missing delete or delete_all permission on product")
