...
async def find_many_access_rule(access: AccessContext, filters: SchemaAccessRuleFilter, session: AsyncSession):
    if "read_all_permission" in access.permissions:
        return await AccessRuleDAO.find_many(filters=filters, session=session)
    if "read_permission" in access.permissions:
        return await AccessRuleDAO.find_many(filters=filters, session=session)
    raise PermissionDenied(custom_detail="Missing read or read_all permission on access_rule")
...
async def update_one_access_rule(
    ...
    if "update_all_permission" in access.permissions:
        return await AccessRuleDAO.update_one(model_id=access_rule_id, session=session, values=filters_dict)
    if "update_permission" in access.permissions:
        return await AccessRuleDAO.update_one(model_id=access_rule_id, session=session, values=filters_dict)
    raise PermissionDenied(custom_detail="Missing update or update_all permission on access_rule")
