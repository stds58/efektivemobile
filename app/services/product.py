...
from app.schemas.product import SchemaProductCreate, SchemaProductFilter, SchemaProductPatch
...
async def find_many_product(access: AccessContext, filters: SchemaProductFilter, session: AsyncSession):
    if "read_all_permission" in access.permissions:
        return await ProductDAO.find_many(filters=filters, session=session)
...
async def update_one_product(
    ...
    if "update_all_permission" in access.permissions:
        return await ProductDAO.update_one(model_id=product_id, session=session, values=filters_dict)
