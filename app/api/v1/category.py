...
async def get_categorys(
    ...
    return await find_many_category(
...
async def edit_category(
    ...
    updated_category = await update_one_category(
        access=access, data=data, session=session, category_id=category_id
    )
    return updated_category


@router.delete(
    "/{id}", summary="Delete category", status_code=status.HTTP_204_NO_CONTENT
)
async def delete_category(
    ...
    await delete_one_category(access=access, session=session, category_id=category_id)
    return
